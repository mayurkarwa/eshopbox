...
function confirm_another_blog_signup( $domain, $path, $blog_title, $user_name, $user_email = '', $meta = array() ) {
	...
	<p>
		<?php printf( __( '<a href="http://%1$s">http://%2$s</a> is your new site. <a href="%3$s">Log in</a> as &#8220;%4$s&#8221; using your existing password.' ), $domain.$path, $domain.$path, "http://" . $domain.$path . "eshop-login.php", $user_name ) ?>
	</p>
	<?php
	/**
...
if ( $active_signup == 'none' ) {
	...
	$login_url = site_url( 'eshop-login.php?redirect_to=' . urlencode( network_site_url( 'wp-signup.php' ) ) );
