...
function preview_theme_ob_filter_callback( $matches ) {
	...
	if (
		( false !== strpos($matches[3], '/eshop-admin/') )
	||
...
function switch_theme( $stylesheet ) {
	...
	if ( is_admin() && false === get_option( 'theme_mods_' . $stylesheet ) ) {
		 ...
		 * the widget customizer when accessing eshop-admin/widgets.php.
		 */
		if ( 'wp_ajax_customize_save' === current_action() ) {
...
function get_theme_mod( $name, $default = false ) {
	...
	/** This filter is documented in eshop-includes/theme.php */
...
function _custom_header_background_just_in_time() {
	...
	if ( current_theme_supports( 'custom-header' ) ) {
		...
		if ( is_admin() ) {
			require_once( ABSPATH . 'eshop-admin/custom-header.php' );
	...
	if ( current_theme_supports( 'custom-background' ) ) {
		...
		if ( is_admin() ) {
			require_once( ABSPATH . 'eshop-admin/custom-background.php' );
...
function check_theme_switched() {
	if ( $stylesheet = get_option( 'theme_switched' ) ) {
		...
		if ( $old_theme->exists() ) {
			...
			/** This action is documented in eshop-includes/theme.php */
 ...
 *
 * Fires when ?wp_customize=on or on eshop-admin/customize.php.
 *
 * @since 3.4.0
 */
