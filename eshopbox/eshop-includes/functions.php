...
function do_robots() {
	...
	if ( '0' == $public ) {
		...
		$output .= "Disallow: $path/eshop-admin/\n";
		$output .= "Disallow: $path/eshop-includes/\n";
	}
 ...
 *
 * Will remove eshop-admin links to retrieve only return URLs not in the eshop-admin
 * directory.
 *
 * @since 2.6.0
 *
...
function wp_guess_url() {
	if ( defined('WP_SITEURL') && '' != WP_SITEURL ) {
		...
		if ( strpos( $_SERVER['REQUEST_URI'], 'eshop-admin' ) !== false || strpos( $_SERVER['REQUEST_URI'], 'eshop-login.php' ) !== false ) {
			$path = preg_replace( '#/(eshop-admin/.*|eshop-login.php)#i', '', $_SERVER['REQUEST_URI'] );
...
/**
 * Output the HTML that shows the eshop-login dialog when the user is no longer logged in.
 *
 * @since 3.6.0
 */
