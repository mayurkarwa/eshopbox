...
if ( is_admin() ) {
	// eshop-admin pages are checked more carefully
	if ( is_network_admin() )
		preg_match('#/eshop-admin/network/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches);
	elseif ( is_user_admin() )
		preg_match('#/eshop-admin/user/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches);
	else
		preg_match('#/eshop-admin/?(.*?)$#i', $_SERVER['PHP_SELF'], $self_matches);
	...
	if ( '' === $pagenow || 'index' === $pagenow || 'index.php' === $pagenow ) {
		...
		if ( '.php' !== substr($pagenow, -4, 4) )
			$pagenow .= '.php'; // for Options +Multiviews: /eshop-admin/themes/index.php (themes.php is queried)
	}
} else {
