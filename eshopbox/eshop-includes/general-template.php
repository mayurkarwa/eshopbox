...
function wp_loginout($redirect = '', $echo = true) {
	...
	if ( $echo ) {
		...
		/** This filter is documented in eshop-includes/general-template.php */
...
function wp_logout_url($redirect = '') {
	...
	$logout_url = add_query_arg($args, site_url('eshop-login.php', 'login'));
...
function wp_login_url($redirect = '', $force_reauth = false) {
	$login_url = site_url('eshop-login.php', 'login');
...
function wp_registration_url() {
	 ...
	 */
	return apply_filters( 'register_url', site_url( 'eshop-login.php?action=register', 'login' ) );
}
...
function wp_login_form( $args = array() ) {
	...
	$form = '
		<form name="' . $args['form_id'] . '" id="' . $args['form_id'] . '" action="' . esc_url( site_url( 'eshop-login.php', 'login_post' ) ) . '" method="post">
			' . $login_form_top . '
...
function wp_lostpassword_url( $redirect = '' ) {
	...
	$lostpassword_url = add_query_arg( $args, network_site_url('eshop-login.php', 'login') );
			...
			foreach ( (array) $results as $result ) {
				if ( $result->post_date != '0000-00-00 00:00:00' ) {
					...
					if ( $result->post_title ) {
						/** This filter is documented in eshop-includes/post-template.php */
...
function wlwmanifest_link() {
	echo '<link rel="wlwmanifest" type="application/wlwmanifest+xml" href="'
		. get_bloginfo('wpurl') . '/eshop-includes/wlwmanifest.xml" /> ' . "\n";
}
 ...
 *
 * @see eshop-includes/class-wp-editor.php
 * @since 3.3.0
 *
 ...
 * @since 2.3.0
 *
 * @param string $file file relative to eshop-admin/ without its ".css" extension.
 */
function wp_admin_css_uri( $file = 'eshop-admin' ) {
 ...
 * $file is a file relative to eshop-admin/ without its ".css" extension. A
 ...
 * 	 to eshop-admin/. Defaults to 'eshop-admin'.
 * @param bool $force_echo Optional. Force the stylesheet link to be printed rather than enqueued.
 */
function wp_admin_css( $file = 'eshop-admin', $force_echo = false ) {
	global $wp_styles;
	 ...
	 *                     relative to eshop-admin/. Defaults to 'eshop-admin'.
	 */
