...
if ( typeof(jQuery) != 'undefined' ) {
	if ( typeof(jQuery.fn.hoverIntent) == 'undefined' ) {
		/* jshint ignore:start */
		// hoverIntent r6 - Copy of eshop-includes/js/hoverIntent.min.js
	...
	jQuery(document).ready(function($){
		...
		touchOpen = function(unbind) {
			adminbar.find('li.menupop').on('click.wp-mobile-hover', function(e) {
				...
				if ( el.parent().is('#eshop-admin-bar-root-default') && !el.hasClass('hover') ) {
					e.preventDefault();
		...
		$('#eshop-admin-bar-get-shortlink').click(function(e){
			e.preventDefault();
			$(this).addClass('selected').children('.shortlink-input').blur(function(){
				$(this).parents('#eshop-admin-bar-get-shortlink').removeClass('selected');
			}).focus().select();
		});
		...
		$('#wpadminbar').click( function(e) {
			if ( e.target.id != 'wpadminbar' && e.target.id != 'eshop-admin-bar-top-secondary' )
				return;
		...
		if ( 'sessionStorage' in window ) {
			$('#eshop-admin-bar-logout a').click( function() {
				try {
	...
	(function(d, w) {
		...
		clickShortlink = function(e) {
			...
			while ( true ) {
				...
				if ( t.id && t.id == 'eshop-admin-bar-get-shortlink' )
					break;
				t = t.parentNode;
			}
		...
		scrollToTop = function(t) {
			...
			if ( t.id != 'wpadminbar' && t.id != 'eshop-admin-bar-top-secondary' )
				return;
		...
		addEvent(w, 'load', function() {
			...
			if ( d.body && aB ) {
				...
				addEvent( document.getElementById('eshop-admin-bar-logout'), 'click', function() {
					if ( 'sessionStorage' in window ) {
						try {
							for ( var key in sessionStorage ) {
								if ( key.indexOf('wp-autosave-') != -1 )
