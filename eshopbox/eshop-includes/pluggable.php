...
function wp_set_auth_cookie($user_id, $remember = false, $secure = '') {
	if ( $remember ) {
		...
		/** This filter is documented in eshop-includes/pluggable.php */
...
function auth_redirect() {
	...
	if ( $secure && !is_ssl() && false !== strpos($_SERVER['REQUEST_URI'], 'eshop-admin') ) {
	...
	if ( $user_id = wp_validate_auth_cookie( '',  $scheme) ) {
		...
		if ( !$secure && get_user_option('use_ssl', $user_id) && false !== strpos($_SERVER['REQUEST_URI'], 'eshop-admin') ) {
			if ( 0 === strpos( $_SERVER['REQUEST_URI'], 'http' ) ) {
				wp_redirect( set_url_scheme( $_SERVER['REQUEST_URI'], 'https' ) );
				exit();
			} else {
 ...
 * list.
 *
 * If the host is not allowed, then the redirect is to eshop-admin on the siteurl
