...
<configuration>
        ...
        <rewrite>
            <rules>
                ...
                <rule name="WordPress Rule for Files" stopProcessing="true">
                    ...
                    <action type="Rewrite" url="' . $iis_rewrite_base . 'eshop-includes/ms-files.php?file={R:1}" appendQueryString="false" />
                </rule>';
                }
                $web_config_file .= '
                <rule name="WordPress Rule 2" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . 'eshop-admin$" ignoreCase="false" />
                    <action type="Redirect" url="' . $iis_subdir_replacement . 'eshop-admin/" redirectType="Permanent" />
                </rule>
	...
	<?php else : // end iis7_supports_permalinks(). construct an htaccess file instead:
		...
		if ( is_multisite() && get_site_option( 'ms_files_rewriting' ) ) {
			$ms_files_rewriting = "\n# uploaded files\nRewriteRule ^";
			$ms_files_rewriting .= $subdir_match . "files/(.+) {$rewrite_base}eshop-includes/ms-files.php?file={$subdir_replacement_12} [L]" . "\n";
		}
...
{$ms_files_rewriting}
# add a trailing slash to /eshop-admin
RewriteRule ^{$subdir_match}eshop-admin$ {$subdir_replacement_01}eshop-admin/ [R=301,L]
...
<?php echo esc_textarea( $htaccess_file ); ?></textarea></li>
	...
	if ( !is_multisite() ) { ?>
		<p><?php printf( __( 'Once you complete these steps, your network is enabled and configured. You will have to log in again.') ); ?> <a href="<?php echo esc_url( site_url( 'eshop-login.php' ) ); ?>"><?php _e( 'Log In' ); ?></a></p>
<?php
	}
}
...
if ( $_POST ) {
	...
	require_once( ABSPATH . 'eshop-admin/includes/upgrade.php' );
