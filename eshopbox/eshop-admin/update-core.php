...
function core_upgrade_preamble() {
	...
	if ( !isset($updates[0]->response) || 'latest' == $updates[0]->response ) {
		...
		if ( wp_http_supports( array( 'ssl' ) ) ) {
			require_once ABSPATH . 'eshop-admin/includes/class-wp-upgrader.php';
...
function list_plugin_updates() {
	...
	require_once(ABSPATH . 'eshop-admin/includes/plugin-install.php');
...
if ( 'upgrade-core' == $action ) {
	...
	require_once( ABSPATH . 'eshop-admin/admin-footer.php' );
