 ...
 * @var string
 */
$self = preg_replace('|^.*/eshop-admin/network/|i', '', $_SERVER['PHP_SELF']);
$self = preg_replace('|^.*/eshop-admin/|i', '', $self);
...
function _wp_menu_output( $menu, $submenu, $submenu_as_parent = true ) {
	...
	foreach ( $menu as $key => $item ) {
		...
		if ( false !== strpos( $class, 'wp-menu-separator' ) ) {
			...
			if ( ! empty( $menu_hook ) || ( ( 'index.php' != $submenu_items[0][2] ) && file_exists( WP_PLUGIN_DIR . "/$menu_file" ) && ! file_exists( ABSPATH . "/eshop-admin/$menu_file" ) ) ) {
			...
			if ( false !== ( $pos = strpos( $menu_file, '?' ) ) )
				$menu_file = substr( $menu_file, 0, $pos );
			if ( ! empty( $menu_hook ) || ( ( 'index.php' != $item[2] ) && file_exists( WP_PLUGIN_DIR . "/$menu_file" ) && ! file_exists( ABSPATH . "/eshop-admin/$menu_file" ) ) ) {
				$admin_is_parent = true;
		...
		if ( ! empty( $submenu_items ) ) {
			...
			foreach ( $submenu_items as $sub_key => $sub_item ) {
				...
				if ( ! empty( $menu_hook ) || ( ( 'index.php' != $sub_item[2] ) && file_exists( WP_PLUGIN_DIR . "/$sub_file" ) && ! file_exists( ABSPATH . "/eshop-admin/$sub_file" ) ) ) {
